...
```matlab
addpath(genpath('/path/to/mhrv'));  % mhrv ROOT (the folder that CONTAINS +mhrv)
mhrv_init;                          % load mhrv default parameters (once per session)
addpath(pwd);                       % this repository
worked_example;                     % runs on bundled demo_CAP_ECG_A1.csv -> demo_output.csv
```

> This build of mhrv is **packaged**: the HRV functions live in `+mhrv/+hrv/`
> and are called as `mhrv.hrv.hrv_time`, `mhrv.hrv.hrv_freq`,
> `mhrv.hrv.hrv_nonlinear`, `mhrv.hrv.hrv_fragmentation`. Add the mhrv **root**
> (the folder that *contains* `+mhrv`) to the path — not the `+mhrv` folder
> itself — and initialise once with `mhrv_init` (the scripts do this for you).
